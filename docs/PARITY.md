...
> 2. **The `connxism::Db` seam** → **now real (2026-07-20).** `crates/connxism/src/kv/mod.rs`
>    re-exports `Db`/`Batch`/`KvItem` from the single Fjall backend
>    (`kv/fjall.rs`); everything above the seam is backend-agnostic. (The earlier
>    "does not exist" note was true when RocksDB was named directly; that backend
>    has since been removed and the store is Fjall-only.)
>
...
| WAL + flush/ack semantics | Fjall journal ✅ + explicit `Estate::flush` (memtables + journal sync; `flush` verb / `Client::flush`) + `fsync_writes` config (synced persist per batch) | ✅ |
| Segments + optimizer (merge, vacuum, indexing thresholds) | Fjall background compaction ✅ + manual `Estate::compact` (full-range, per keyspace; `compact` verb) + per-keyspace live segment bytes in health | ✅ core |
