...
use rro_core::{Document, Embedder, Recall, Result, RroError, VectorRecord};
...
impl IngestHandle {
    ...
    pub async fn submit(&self, doc: Document) -> Result<()> {
        self.tx
            .send(doc)
            .await
            .map_err(|_| RroError::msg("ingest machine stopped"))
    }
    ...
    pub async fn finish(self) -> Result<IngestStats> {
        ...
        self.worker
            .await
            .map_err(|e| RroError::msg(format!("ingest worker panicked: {e}")))
    }
}
