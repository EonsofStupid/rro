...
//!
//! One estate == one Fjall database. Everything the engine knows lives in a column
...
/// Maximum key length. Fjall caps keys at 65 536 bytes, so the engine enforces
/// the bound up front — an over-limit key fails cleanly at the call site rather
/// than deep inside the store.
...
/// through transaction-scoped read-modify-write (+1 / −1 deltas folded in at
/// commit) — the stats that make max-score pruning possible without breaking the
/// no-read-modify-write law on the posting rows themselves.
