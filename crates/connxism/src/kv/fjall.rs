...
//!
//! The sole KV backend behind the seam (see
//! [`crate::kv`]). It maps connxism's storage needs onto Fjall 3.1.7:
...
//!     no data migration and the `i64 LE` counter format stays stable.
//!
...
//!
//! ## Capability parity with RocksDB
//!
...
/// Data block size — 16 KiB blocks (the same unit as RocksDB's
/// `set_block_size(16*1024)`), the unit the shared block cache holds and evicts.
/// Fjall drops to its default block size otherwise; setting it explicitly keeps
/// the cache granularity a decision rather than an inherited default.
...
/// Per-keyspace tuning: a point-lookup bloom, compression, per-CF memtable size,
/// and KV separation on the vector CFs.
fn keyspace_options(name: &str, memtable_bytes: u64) -> KeyspaceCreateOptions {
    ...
    let mut opts = KeyspaceCreateOptions::default()
        ...
        // 16 KiB data blocks on every CF (the analogue of RocksDB's
        // `set_block_size`), set explicitly so the cache granularity is a decision.
...
/// name so batched merges can key their accumulator by CF.
#[derive(Clone, Copy)]
...
impl Db {
    /// Open (or create) the estate's Fjall database at `path`, applying the
    /// estate's per-CF tuning: a shared block/blob cache, a
    ...
    pub(crate) fn open(path: &Path, config: &EstateConfig) -> Result<Db> {
        ...
        // total write memory is bounded by that × the live keyspaces — the
        // budget RocksDB would state explicitly via `db_write_buffer_size`.
