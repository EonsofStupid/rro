...
/// store (Fjall, behind the `kv` seam).
pub struct Estate {
    pub(crate) db: Db,
...
impl Estate {
    ...
    ///
    /// Consistency: the store copies the directory under a held MVCC snapshot.
    /// This requires the writer quiescent, so this drains the out-of-band graph
    /// applier before copying.
...
impl Estate {
    ...
    /// operator-invoked optimizer pass (Fjall runs its own background
