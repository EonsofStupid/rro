...
//! fusion. All Fjall work runs on the blocking pool so the tokio runtime
...
fn upsert_into(
    ...
    for r in records {
        ...
        // Key-length guard (Fjall caps keys at 65536 bytes): a doc id keys
        // `docs`/`vecs`/postings directly, so an over-limit id is rejected up
        // front rather than deep inside the store.
