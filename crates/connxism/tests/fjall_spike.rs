//! How connxism uses Fjall 3.x for its four storage needs (`docs/FJALL_MIGRATION_PLAN.md`).
//!
//! connxism leans on the store for four things, all funnelled through the `Db`
...
//!
//! This exercises the real Fjall 3.1.7 API these four needs rest on, and pins the
//! `tdf` mechanism (a transaction-scoped read-modify-write) as a test, not a wiki.
//!
...
fn keyspaces_are_column_families_and_survive_restart() {
    ...
    // One Fjall keyspace per logical column family.
    let nodes = db
